...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const aboutPrevious = document.querySelector(
    ".about-nav-previous"
  );
  const aboutNext = document.querySelector(
    ".about-nav-next"
  );
  ...
  if (
    ...
    const cards = Array.from(
      aboutCards.querySelectorAll(".about-card")
    );

    let currentAboutCard = 0;

    function updateAboutNavigation(index) {
      currentAboutCard = Math.max(
        0,
        Math.min(index, cards.length - 1)
      );

      aboutPrevious.disabled = currentAboutCard === 0;
      aboutNext.disabled =
        currentAboutCard === cards.length - 1;

      aboutDots.forEach((dot, dotIndex) => {
        dot.classList.toggle(
          "is-active",
          dotIndex === currentAboutCard
        );
      });
    }
    ...
    function goToAboutCard(index) {
      ...
      updateAboutNavigation(index);
    }

    aboutPrevious.addEventListener("click", () => {
      goToAboutCard(currentAboutCard - 1);
    });
    ...
    aboutDots.forEach((dot) => {
      dot.addEventListener("click", () => {
        goToAboutCard(
          Number(dot.dataset.aboutIndex)
        );
      });
    });

    aboutCards.addEventListener(
      "scroll",
      () => {
        ...
        const visibleIndex = Math.round(
          aboutCards.scrollLeft / cardWidth
        );

        updateAboutNavigation(visibleIndex);
      },
      { passive: true }
    );

    updateAboutNavigation(0);
  }
});
